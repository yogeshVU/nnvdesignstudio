...
define([
    ...
    ParamsWidget.prototype._initialize = function () {
        ...
        this.createNewBtn.on('click', () => {
            if (this.readOnly || this.isinstance) {
                return;
            }
            ...
            nameInput.editInPlace({
                ...
                onChange: (oldValue, newValue) => {
                    if (this.checkAttributeName(newValue))
                    {
                        gmeid = this.addNewAttribute(newValue, '', 'String');
                    
                        if (gmeid)
                        {
                            text = newValue;
                            this.paramDesc.parameters[gmeid]=[text,'','String'];
                ...
                onFinish: () => {
                    ...
                    if (gmeid)
                    {
                        this.addAttribute(gmeid,[text,'','String']);
        ...
        header.append($('<th>', {text: 'Value', class: 'header-desc'}));
        //header.append($('<th>', {text: 'Type', class: 'header-asset'}));
        ...
        this.tableBody.on('click', '.delete-btn', (event) => {
            ...
            const attrGMEID = $(event.target).closest('tr').data('gmeid');
            const attrName  = this.attrs[attrGMEID].name;
            ...
            (new ConfirmDialog()).show({deleteItem: attrName}, () => {
                this.deleteAttribute(attrGMEID,attrName);
                rowValue.remove();
                this.removeAttribute(attrGMEID);
            });
        ...
        this.tableBody.on('dblclick', '.row-name', (event) => {
            ...
            const attrGMEID = trEl.closest('tr').data('gmeid');
            const attrName  = this.attrs[attrGMEID].name;
            var cvalue =attrName;
            if (this.readOnly || this.isinstance) {
                return;
            ...
            trEl.editInPlace({
                ...
                onChange: (oldValue, newValue) => {
                    if (this.checkAttributeName(newValue))
                    {
                        this.renameAttribute(attrGMEID, oldValue, newValue);
                        cvalue = newValue;
                        this.paramDesc.parameters[attrGMEID][0]=newValue;
                        this.attrs[attrGMEID].name = newValue;
                    }
                },
                onFinish: () => {
        ...
        this.tableBody.on('dblclick', '.row-desc', (event) => {
            ...
            trEl.editInPlace({
                ...
                enableEmpty: true,
                onChange: (oldValue, newValue) => {
                    this.updateAttributeValue(attrGMEID, attrName, newValue);
                    desc = newValue;
                    this.paramDesc.parameters[attrGMEID][1]=newValue;
                },
                onFinish: () => {
                    // Wait for the new value to be accepted..
                    trEl.text(desc);
        ...
        this.tableBody.on('dblclick', '.row-asset', (event) => {
            ...
            trEl.editInPlace({
                ...
                enableEmpty: true,
                onChange: (oldValue, newValue) => {
                    //this.updateAttributeType(attrGMEID, attrName, newValue);
                    desc = newValue;
                },
                onFinish: () => {
                    // Wait for the new value to be accepted..
    ...
    ParamsWidget.prototype.checkAttributeName = function (newname) {
        var self = this;
        var keys = Object.keys(self.attrs);
        var i =0;
        for(i=0; i!=keys.length; i+=1)
        {
            if (self.attrs[keys[i]].name == newname)
            {
                return false;
            }
        }
    ...
    ParamsWidget.prototype.addNode = function (desc) {
        ...
        if (desc) {
            ...
            {
                self.paramDesc =  desc;
                self.isinstance = self.paramDesc.isinstance;
    ...
    ParamsWidget.prototype.renderTable = function () {
        ...
        var keys = Object.keys(self.paramDesc.parameters);
        var i = 0;
        for(i = 0; i!= keys.length; i+=1)
        {
            self.addAttribute(keys[i], self.paramDesc.parameters[keys[i]]);
        }
        this.applySortAndFilters();
    }

    ParamsWidget.prototype.addAttribute = function (id, info) {
        var name =  info[0];
        var value = info[1];
        var type = info[2];
   ...
   /* ParamsWidget.prototype.updateAttributeValue = function (attr) {
    ...
    ParamsWidget.prototype.updateAttributeType = function (attr) {
        const attrEl = this.attrs[attr.name].el;

        if (this.attrs[attr.name].type !== attr.type) {
            this.attrs[attr.name].assetWidget.setValue(attr.value);
            this.attrs[attr.name].value = attr.value;
    ...
    ParamsWidget.prototype.removeAttribute = function (attrID) {
        this.attrs[attrID].el.remove();
        delete this.attrs[attrID];
        delete this.paramDesc.parameters[attrID];
    };
